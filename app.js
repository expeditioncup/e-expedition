...
// Só reinicia a tela quando o usuário realmente muda (entrou ou saiu).
// Sem isso, o Supabase dispara SIGNED_IN toda vez que a aba volta ao foco
// e o site voltava sozinho para a aba "Tabela".
db.auth.onAuthStateChange((_evento, sessao) => {
  const novo = sessao?.user ?? null;
  if (estado.iniciado && (novo?.id ?? null) === (estado.usuario?.id ?? null)) return;
  estado.usuario = novo;
  estado.iniciado = true;
  setTimeout(iniciar, 0); // fora do callback, para não travar o lock do Supabase
});

function esconderCarregando() {
  // remove() em vez de hidden: funciona mesmo se o CSS sobrescrever [hidden]
  $("#carregando")?.remove();
}

// Rede de segurança: se em 8 s nada tirou o "Carregando…", mostra o motivo provável.
setTimeout(() => {
  if (!$("#carregando")) return;
  esconderCarregando();
  $("#app").hidden = true;
  $("#tela-login").hidden = false;
  mostrarErro("Demorou demais para falar com o Supabase. Confira a URL, a chave e se o projeto não está pausado. Depois recarregue a página.");
}, 8000);
...
async function iniciar() {
  try {
    ...
    $("#app").hidden = true;
    $("#tela-login").hidden = false;
    mostrarErro("Erro ao carregar os dados: " + (e?.message ?? e));
  }
}
...
async function iniciarApp() {
  ...
  const { data: perfil, error: erroPerfil } = await db.from("profiles").select("*").eq("id", estado.usuario.id).single();
  if (erroPerfil) throw new Error("Não achei seu perfil (" + erroPerfil.message + "). O schema.sql foi rodado neste projeto?");
